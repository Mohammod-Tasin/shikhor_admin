...
interface AdmitCardUploadResponse {
  id: string;
 ...
 * the previous one). Responds with just `{ id, admit_card_url }`, not the
 * full registration row. Bearer token and multipart boundary are handled by
 * `apiFetch`.
 */
export function uploadAdmitCard(id: string, file: File) {
  const form = new FormData();
  form.append("file", file);
  return apiFetch<AdmitCardUploadResponse>(
